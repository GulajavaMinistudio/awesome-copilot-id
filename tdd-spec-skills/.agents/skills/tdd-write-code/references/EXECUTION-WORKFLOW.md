...
This document serves as the mandatory technical reference for the `/tdd-write-code` execution process. It defines the operational workflow, task management, environment setup, and version control rules.

## 1. Workflow (Integrated Refactoring)

1. **Analyze & Plan (The Blueprint)**:
    * **Read Guidelines**: Check `memory.instructions.md or CONSTRAINTS.md` for specific coding guidelines.
...
2. **Develop a Detailed Plan**: Outline a clear, step-by-step todo list using the `#todos` tool, strictly driven by the verifiable goals outlined in `KARPATHY-GUIDELINES.md` (Think Before Coding, Simplicity First, Surgical Changes, Goal-Driven Execution).
