...
3. **Core Rules Discovery:** Read the active platform's corresponding agent definition file for detailed constraints:
   - Path: .commandcode/agents/ClarificationAnalyst.md
...
- **Fuzzy Language:** Spot overloaded or imprecise terminology and propose canonical terms. When a canonical term is chosen, ensure rejected synonyms are listed under `_Avoid_` as defined in `.commandcode/standards/CONTEXT-FORMAT.md`.
...
You **MUST** use the mandatory clarification report template format when generating the final summary. Read the template from: `.commandcode/skills/clarification-analyst/references/CLARIFICATION-REPORT-TEMPLATE.md`
