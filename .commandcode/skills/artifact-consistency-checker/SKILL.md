...
- **Enforce Documentation Standards:** Always compare the generated documents against `.commandcode/standards/ADR-FORMAT.md` and `CONTEXT-FORMAT.md`. If a document structure is "broken" or does not match the mandatory template, list it as a "Consistency Failure" in the Audit Report.
