...
3. **Core Rules Discovery:** Read the active platform's corresponding agent definition file for detailed constraints:
   - Path: .omp/agents/Grilling (or .omp/rules/grilling.md or .omp/agents/grilling.toml)
...
1. **Domain Glossary Integration:**
   ...
   - Record the chosen canonical term and list rejected synonyms under `_Avoid_` as defined in `.omp/standards/CONTEXT-FORMAT.md`.

2. **Architecture Decision Records (ADRs):**
   If a decision is a "hard-to-reverse" architectural choice:
   - Verify it meets **all three** criteria from `.omp/standards/ADR-FORMAT.md`: (1) Hard to reverse, (2) Surprising without context, (3) Real trade-off.
   - If it does, document it **lazily** as an ADR under `docs/adr/` using the format defined in `.omp/standards/ADR-FORMAT.md`. Do not embed the ADR in other documents.
