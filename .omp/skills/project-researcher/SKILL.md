...
1.  **High-Level Scan:**
    ...
    - **Prior Work Scan:** Read existing files in `docs/` and `spec/` directories to understand previously established architectures, API contracts, and business logic. Also, read any formatting rules in `.omp/standards/` if you need to generate new documentation.
...
2.  **Content:** The file's content **MUST** adhere to the Mandatory Architecture Template. You MUST read this template from `.omp/skills/project-researcher/references/ARCHITECTURE-TEMPLATE.md` before generating the document.
...
1.  Locate the `AGENTS.md` file (or the primary agent configuration/index file in the `.omp/` or root directory).
