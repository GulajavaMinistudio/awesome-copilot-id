...
1. **Mandatory Pre-Flight Architecture Scan:** Before generating any Discovery Drafts or critiquing the architecture, you MUST check for the existence of `docs/ARCHITECTURE.md`. If it does not exist, or if the repository has undergone significant changes since its last update, you MUST invoke the `project-researcher` skill as your very first step to map the repository architecture, and **proactively offer to write or update `docs/ARCHITECTURE.md`** based on the results.
