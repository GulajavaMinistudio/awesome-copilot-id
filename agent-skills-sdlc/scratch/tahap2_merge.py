...
skill_path_old = r"d:\WebstormProject\awesome-copilot-id\.agents\skills\product-manager-prd\SKILL.md"
skill_path_new = r"d:\WebstormProject\awesome-copilot-id\agent-skills-sdlc\.agents\skills\prd\SKILL.md"
...
persona_match = re.search(r'(# Phase 1: Product Requirements Architect \(Senior Product Manager\).*?)(\n## |\Z)', rule_content, re.DOTALL)
persona = persona_match.group(1).strip() if persona_match else ""
persona = persona.replace("# Phase 1: Product Requirements Architect (Senior Product Manager)", "## 🧠 The Senior Product Manager Persona")

core_match = re.search(r'(## Core Directives.*?)(\n## |\Z)', rule_content, re.DOTALL)
core = core_match.group(1).strip() if core_match else ""
# Ensure it uses the emoji for consistency if we want, or just leave it
...
workflow_match = re.search(r'(## ⚙️ Operational Workflow.*?)(\n---|\n## Mandatory PRD Template|\Z)', old_skill, re.DOTALL)
workflow = workflow_match.group(1).strip() if workflow_match else ""

template_match = re.search(r'(## Mandatory PRD Template.*)', old_skill, re.DOTALL)
...
def cleanup_legacy(text):
    ...
    text = text.replace("@SpecificationArchitect (or /specification-architect)", "/spec")
    ...
    text = text.replace("the `product-manager-prd` skill", "this skill")
    return text
...
new_content += "# Product Manager PRD Skill (`/prd`)\n\n"
