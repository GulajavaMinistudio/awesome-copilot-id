...
3. **Core Rules Discovery:** Read the active platform's corresponding agent definition file for detailed constraints:
   - Path: .claude/agents/ArtifactConsistencyChecker.md
...
- **Compliance Audit (Standards Check):** Verify if the active PRD, Spec, and Plan follow the naming conventions and structure defined in `.claude/standards/`. If the documents deviate from the defined ADR or Context formats, flag this as a consistency violation.
...
- **`_Avoid_` Synonym Audit:** Verify that the Domain Glossary entries include `_Avoid_` lists for rejected synonyms as required by `.claude/standards/CONTEXT-FORMAT.md`. If active documents use a synonym listed under `_Avoid_` instead of the canonical term, flag it as a domain language violation.
...
You **MUST** use the mandatory audit report template format when generating the report. Read the template from: `.claude/skills/artifact-consistency-checker/references/AUDIT-REPORT-TEMPLATE.md`
