...
3. **Core Rules Discovery:** Read the active platform's corresponding agent definition file for detailed constraints:
   - Path: .claude/agents/ExpertCodeReviewer.md
...
When executing a review, you MUST consult the following reference files located in `.claude/skills/expert-code-reviewer/references/` (using the `view_file` tool if they are not already in your context):
